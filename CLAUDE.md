...
```
src/
  app/       App (path-based routing: "/" → Landing, "/mountains" → DesignPage,
             "/maps" → SkylinePage, "/pendant" → PendantPage),
  ...
  entities/  mountains (procedural elevation), buildings & streets (Overpass/OSM),
             ring (metals, Shape, design-input → geometry params)
  features/  location-search (Google Geocoder), ring-controls, stl-export
  widgets/   designer, mountains-map (Google Maps), ring-viewer (r3f),
...
**/pendant — image → pendant designer** (independent of the map/mountains flow)
...
- Map, search, and elevation all need `VITE_GOOGLE_MAPS_API_KEY` with billing enabled; without it everything degrades (map notice, procedural mountains).
